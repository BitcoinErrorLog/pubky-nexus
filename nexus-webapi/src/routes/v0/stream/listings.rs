...
use nexus_common::models::marketplace::{ListingSaleFormat, ListingStream, ListingStreamFilters};
...
#[utoipa::path(
    ...
    params(
        ...
        ("start" = Option<usize>, Query, description = "The start of the stream timeframe. Listings with a timestamp greater than this value will be excluded from the results"),
        ("end" = Option<usize>, Query, description = "The end of the stream timeframe. Listings with a timestamp less than this value will be excluded from the results"),
    ),
    responses(
    ...
    description = r#"Stream Listings: Retrieve a stream of marketplace listings sorted by indexing timeline.
...
pub async fn stream_listings_handler(
    ...
    match ListingStream::get_listings(query.filters, query.pagination, order).await? {
...
#[openapi(
    ...
    components(schemas(ListingStream, ListingStreamFilters, SortOrder))
)]
